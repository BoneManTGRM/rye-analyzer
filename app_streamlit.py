# app_streamlit.py
# RYE Analyzer — mobile-friendly Streamlit app
# Multi-format ingest • Presets & tips • Auto column detect • Auto rolling window
# Single / Compare / Multi-domain / Reports • Diagnostics & PDF
...
try:
    from presets import PRESETS as _PRESETS  # preferred
    PRESETS = _PRESETS
    ...
    try:
        ...
        PRESETS = {
            "Generic": type("Preset", (), {
                ...
                "domain": "domain",
                "default_rolling": 10,
                "tooltips": {"Generic": "Basic preset when presets are not available."}
            })()
        }
...
# compute_rye_from_df aliasing
try:
    from core import compute_rye_from_df as _compute_rye_from_df
except Exception:
    from core import compute_rye as _compute_rye_from_df  # type: ignore

# summarize aliasing
try:
    from core import summarize_series as _summarize_series
except Exception:
    from core import summarize as _summarize_series  # type: ignore

# rolling_series fallback
try:
...
# Optional analytics (each may be absent)
try:
...
def ema_series(x, span: int) -> np.ndarray:
    ...
    return s.ewm(span=int(span), adjust=False).mean().values
...
def smart_window(n_rows: int, preset_default: int | None) -> int:
    ...
    if n_rows <= 0:
        return 10
    return max(3, min(200, int(round(max(3, n_rows * 0.05)))))
...
try:
    ...
    if spec is not None:
        ...
        _pdf_import_error = "report.py not found."
except Exception:
...
# ====== PRE-SEED SESSION STATE (prevents key-mutation crash) ======
def _seed(key: str, value):
    if key not in st.session_state:
        st.session_state[key] = value

default_preset_name = list(PRESETS.keys())[0]
preset_for_seed = PRESETS.get(default_preset_name, next(iter(PRESETS.values())))
def _first_or(default, lst): return (lst[0] if isinstance(lst, list) and lst else default)
_seed("preset_name", default_preset_name)
_seed("col_time",   _first_or("time", getattr(preset_for_seed, "time", ["time"])))
_seed("col_domain", getattr(preset_for_seed, "domain", "domain") or "domain")
_seed("col_repair", _first_or("performance", getattr(preset_for_seed, "performance", ["performance"])))
_seed("col_energy", _first_or("energy",     getattr(preset_for_seed, "energy", ["energy"])))
_seed("auto_roll", True)
_seed("ema_span", 0)
...
with st.sidebar:
    ...
    file_types = ["csv","tsv","xls","xlsx","parquet","feather","json","ndjson","h5","hdf5","nc","netcdf","arrow"]
    ...
    # Auto-detect button: update state, then rerun
    ...
    auto_roll = st.checkbox("Auto rolling window", value=st.session_state["auto_roll"],
                            key="auto_roll",
                            help="Use preset default or a smart guess by series length.")
    window = st.number_input("Rolling window", min_value=1, max_value=1000, value=default_window, step=1,
                             ...
                             disabled=st.session_state["auto_roll"])

    ema_span = st.number_input("EMA smoothing (optional)", min_value=0, max_value=1000,
                               value=int(st.session_state["ema_span"]), step=1,
                               help="Extra smoothing; 0 disables EMA.", key="ema_span_widget")
    st.session_state["ema_span"] = int(ema_span)
...
def compute_block(df: pd.DataFrame, label: str, sim_mult: float, auto_roll_flag: bool) -> dict:
    ...
    rye_ema = ema_series(rye, span=int(st.session_state["ema_span"])) if int(st.session_state["ema_span"]) > 1 else None
...
with tab1:
    if df1 is None:
        ...
        if ensure_columns(df1, col_repair, col_energy):
            block = compute_block(df1, "primary", sim_factor, st.session_state["auto_roll"])
            rye = block["rye"]; rye_roll = block["rye_roll"]; rye_ema = block["rye_ema"]; rye_cum = block["rye_cum"]
            w = block["w"]; summary = block["summary"]
            ...
            if col_time in df1.columns:
                ...
                add_stability_bands(fig); st.plotly_chart(fig, use_container_width=True)

                fig2 = px.line(x=idx, y=rye_roll, labels={"x": col_time, "y": f"RYE rolling {w}"},
                               title=f"RYE rolling window {w}")
                if rye_ema is not None:
                    fig2.add_scatter(x=idx, y=rye_ema, mode="lines", name=f"EMA {st.session_state['ema_span']}")
                add_stability_bands(fig2); st.plotly_chart(fig2, use_container_width=True)
                ...
                if rye_ema is not None:
                    ycols[f"EMA {st.session_state['ema_span']}"] = rye_ema
            ...
            with st.expander("Diagnostics"):
                ...
                if detect_regimes is not None:
                    try:
                        ...
                        if regimes:
                            st.write("Detected regimes:"); st.json(regimes)
                    except Exception:
                        pass
            ...
            if rye_ema is not None:
                enriched[f"RYE_ema_{st.session_state['ema_span']}"] = rye_ema
...
with tab2:
    if df1 is None or df2 is None:
        ...
        if ensure_columns(df1, col_repair, col_energy) and ensure_columns(df2, col_repair, col_energy):
            b1 = compute_block(df1, "A", sim_factor, st.session_state["auto_roll"])
            b2 = compute_block(df2, "B", sim_factor, st.session_state["auto_roll"])
...
with tab3:
    if df1 is None:
        ...
        if col_domain not in df1.columns:
            ...
            b = compute_block(df1, "primary", sim_factor, st.session_state["auto_roll"])
            dfp = b["df"].copy(); dfp["RYE"] = b["rye"]
