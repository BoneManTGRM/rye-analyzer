...
# ---------------- Core workers (moved up so sidebar can use load_any) ----------------
...
def load_any(file) -> Optional[pd.DataFrame]:
    ...
    try:
        ...
        elif name_lower.endswith(".geojson") or (
            name_lower.endswith(".json") and "geojson" in name_lower
        ):
            try:
...
with st.sidebar:
    ...
    # Preview dataframe for column inference: use load_any on a copy so gz/zip work
    df_preview: Optional[pd.DataFrame] = None
    if file1 is not None:
        try:
            preview_buf = io.BytesIO(file1.getbuffer())
            preview_buf.name = getattr(file1, "name", "preview")  # type: ignore[attr-defined]
            df_preview = load_any(preview_buf)
        except Exception:
            df_preview = None
